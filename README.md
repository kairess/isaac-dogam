...
- 색깔 칩을 누르면 걸러내지 않고 **그 자리로 데려다 줍니다.** 내리는 동안
  ...
  (칩은 무지개 여섯에 갈색·흰색·검정을 더한 아홉 개. 분홍·살구·회색은
  이정표 없이 그 사이사이에 놓여 있고, 그 자리에서는 바로 앞서 지나온 칩이
  켜져 있습니다 — 목차가 소제목 자리에서도 큰 제목을 켜 두는 것처럼)
...
- 제목 옆에 지금 기기가 물고 있는 **판 번호**(`v21`)가 보입니다. **제목을 누르면 새로고침**
...
색깔 묶음의 순서는 빨강부터 검정까지 고정입니다. 묶음 **안**은 두 단계로 세웁니다
(`tools/build.py` 의 `order`).

1. **잇기** — 앞 아이콘과 색이 가장 가까운 것을 차례로 집어 붙입니다 (`greedy`).
2. **풀기** — 탐욕법은 되돌아갈 줄 몰라서, 한 무리를 다 써 버리면 멀리 남겨 둔
   아이콘으로 순간이동합니다. 그 막다른 길을 구간 뒤집기로 풉니다 (`two_opt`).

색이 얼마나 다른지는 **CIEDE2000** 으로 잽니다 (`extract_colors.gap`). CIELAB 위에서
그냥 유클리드 거리(CIE76)를 쓰면 파랑과 진한 색에서 사람 눈과 어긋납니다.
Sharma 검증표 16쌍으로 구현을 맞춰 두었습니다.

무채색은 이어 붙이지 않고 **밝은 것부터** 놓습니다. 밝기는 CIELAB 의 `L` 로 재는데,
HSV 의 `V` 는 가장 밝은 채널 하나만 보는 값이라 사람이 느끼는 밝기와 어긋납니다.
흰색 → 회색 → 검정이 그대로 이어지고, 도감의 맨 끝이 가장 어두운 아이콘이 됩니다.

이웃한 두 아이콘의 색차(ΔE00)가 이렇게 줄었습니다.

| | 묶음별 비중순 | 잇기만 | **지금** |
|---|---|---|---|
| 유채색 | 5.1 | 3.84 | **3.24** |
| 무채색 | 5.7 | 5.73 | **5.01** |
| 눈에 띄는 단절(20 초과) | — | 17곳 | **9곳** |
| 무채색 밝기 오르내림 | — | 8.6배 | **1.2배** |

묶음은 여전히 한 덩어리라, 색깔 칩이 **그 자리로 뛰기**를 할 수 있습니다.
빌드는 이 계산에 약 6초를 씁니다.

> 아이콘마다 **버금색**까지 들고 거리를 재 보기도 했는데, 해당하는 아이콘이 80개뿐인데다
> 으뜸색 기준 색차가 오히려 나빠져서(3.24 → 3.32) 넣지 않았습니다.
